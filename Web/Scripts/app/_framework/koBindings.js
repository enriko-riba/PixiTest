    ...
    ko.bindingHandlers.datalist = (function () {
        ...
        return {
            ...
            init: function (element, valueAccessor, allBindingsAccessor) {
                var setup = valueAccessor(), textProperty = ko.unwrap(setup.optionsText), valueProperty = ko.unwrap(setup.optionsValue), dataItems = ko.unwrap(setup.options), myValue = setup.value, datalist = document.createElement("DATALIST"), shouldSet = false;
                ...
                function updateModel() {
                    ...
                    var newVal = element.value, dataItems = ko.unwrap(setup.options), selectedItem = findItem(dataItems, textProperty, newVal), newValue = selectedItem ? getVal(selectedItem, valueProperty) : newVal;
                    if (ko.isWriteableObservable(myValue)) {
                        myValue(newValue);
                    }
                }
                function updateView() {
                    var modelValue = ko.unwrap(myValue), dataItems = ko.unwrap(setup.options), selectedItem = findItem(dataItems, valueProperty, modelValue), newValue = selectedItem ? getVal(selectedItem, textProperty) : undefined;
            ...
            update: function (element, valueAccessor) {
                var setup = valueAccessor(), datalist = element.list, dataItems = ko.unwrap(setup.options), textProperty = ko.unwrap(setup.optionsText), valueProperty = ko.unwrap(setup.optionsValue), descriptionProperty = ko.unwrap(setup.optionsDescription);
